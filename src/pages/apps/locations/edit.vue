<template>
  <div class="d-flex justify-between align-center mb-4">
    <h3>Edit Location</h3>
    <VBtn variant="text" @click="$router.push('/dashboards/locations')">Back to list</VBtn>
  </div>
...
const fetchLocation = async () => {
  try {
    ...
    const res = await axios.get(`${apiBaseUrl}/locations/${locationId}`, {
      headers: {
    ...
    message.value = error.response?.data?.message || 'Failed to load location.'
  } finally {
    loading.value = false
  }
}
...
const submitForm = async () => {
  try {
    ...
    await axios.put(`${apiBaseUrl}/locations/${locationId}`, payload, {
      headers: {
    ...
    if (error.response?.status === 422 && error.response?.data?.errors) {
      ...
      message.value = error.response?.data?.message || 'Failed to update location.'
    }
  } finally {
    saving.value = false
  }
