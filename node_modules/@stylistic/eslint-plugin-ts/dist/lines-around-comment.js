...
const baseRule = blockSpacing.getESLintCoreRule("lines-around-comment");
...
var linesAroundComment = blockSpacing.createRule({
  ...
  create(context, [_options]) {
    ...
    function isCommentAtInterfaceStart(token) {
      return isCommentAtParentStart(token, utils.AST_NODE_TYPES.TSInterfaceBody);
    }
    function isCommentAtInterfaceEnd(token) {
      return isCommentAtParentEnd(token, utils.AST_NODE_TYPES.TSInterfaceBody);
    }
    function isCommentAtTypeStart(token) {
      return isCommentAtParentStart(token, utils.AST_NODE_TYPES.TSTypeLiteral);
    }
    function isCommentAtTypeEnd(token) {
      return isCommentAtParentEnd(token, utils.AST_NODE_TYPES.TSTypeLiteral);
    }
    function isCommentAtEnumStart(token) {
      return isCommentAtParentStart(token, utils.AST_NODE_TYPES.TSEnumDeclaration);
    }
    function isCommentAtEnumEnd(token) {
      return isCommentAtParentEnd(token, utils.AST_NODE_TYPES.TSEnumDeclaration);
    }
    function isCommentAtModuleStart(token) {
      return isCommentAtParentStart(token, utils.AST_NODE_TYPES.TSModuleBlock);
    }
    function isCommentAtModuleEnd(token) {
      return isCommentAtParentEnd(token, utils.AST_NODE_TYPES.TSModuleBlock);
    }
    ...
    const customReport = (descriptor) => {
      if ("node" in descriptor) {
        if (descriptor.node.type === utils.AST_TOKEN_TYPES.Line || descriptor.node.type === utils.AST_TOKEN_TYPES.Block) {
    ...
    return {
      Program() {
        rules.Program();
        comments.forEach((token) => {
          if (token.type === utils.AST_TOKEN_TYPES.Line) {
          ...
          } else if (token.type === utils.AST_TOKEN_TYPES.Block) {
