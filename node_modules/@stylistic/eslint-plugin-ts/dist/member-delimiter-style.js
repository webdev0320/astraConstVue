...
var memberDelimiterStyle = blockSpacing.createRule({
  ...
  create(context, [options]) {
    ...
    const interfaceOptions = blockSpacing.deepMerge(
      baseOptions,
      overrides.interface
    );
    const typeLiteralOptions = blockSpacing.deepMerge(
      baseOptions,
    ...
    function checkMemberSeparatorStyle(node) {
      const members = node.type === utils.AST_NODE_TYPES.TSInterfaceBody ? node.body : node.members;
      ...
      const typeOpts = node.type === utils.AST_NODE_TYPES.TSInterfaceBody ? interfaceOptions : typeLiteralOptions;
