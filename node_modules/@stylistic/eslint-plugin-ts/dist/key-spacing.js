...
const baseRule = blockSpacing.getESLintCoreRule("key-spacing");
...
var keySpacing = blockSpacing.createRule({
  name: "key-spacing",
  meta: {
    type: "layout",
    docs: {
  ...
  create(context, [options]) {
    ...
    function adjustedColumn(position) {
      ...
      return blockSpacing.getStringLength(
    ...
    function isKeyTypeNode(node) {
      return (node.type === utils.AST_NODE_TYPES.TSPropertySignature || node.type === utils.AST_NODE_TYPES.TSIndexSignature || node.type === utils.AST_NODE_TYPES.PropertyDefinition) && !!node.typeAnnotation;
    }
    ...
    function getKeyText(node) {
      if (node.type !== utils.AST_NODE_TYPES.TSIndexSignature)
    ...
    function getKeyLocEnd(node) {
      return getLastTokenBeforeColon(
        node.type !== utils.AST_NODE_TYPES.TSIndexSignature ? node.key : at(node.parameters, -1)
      ).loc.end;
    }
    ...
    function validateBody(body) {
      ...
      const members = body.type === utils.AST_NODE_TYPES.TSTypeLiteral ? body.members : body.body;
      let alignGroups = [];
